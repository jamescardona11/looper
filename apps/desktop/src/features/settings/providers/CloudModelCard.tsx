...
import ModelCardShell, { waveDots } from "../models/ModelCardShell";

export type CloudModelCardProps = {
...
const CLOUD_ICON = [
  ...
  "bg-[color-mix(in_srgb,var(--color-cloud)_12%,transparent)] ui-color-cloud",
].join(" ");
const NAVIGATION_ICON = [
  "grid h-7 w-7 shrink-0 place-items-center rounded-full",
  "border border-border-secondary text-content-disabled transition",
  "group-hover:border-[var(--color-cloud)] group-hover:text-[var(--color-cloud)]",
].join(" ");
...
export function CloudModelCard({
  ...
  return (
    <ModelCardShell
      ...
      dots={waveDots(
        ["cloud", providerLabel, modelLabel ?? "default"].join("/"),
      )}
      width={width}
      onClick={onClick}
      ariaLabel={t({
    ...
    >
      <CloudCardDetails
        ...
        navigable={Boolean(onClick)}
      />
    </ModelCardShell>
  );
}

function CloudCardDetails({
  provider,
  description,
  navigable,
}: {
