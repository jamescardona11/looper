...
describe("home time periods", () => {
  ...
  test("keeps a deterministic choice within each local period", () => {
    ...
    expect(
      pickStableForCurrentPeriod(choices, 7, new Date("2024-02-29T08:00:00")),
    ).toBe("four");
    expect(
      pickStableForCurrentPeriod(choices, 7, new Date("2024-02-29T13:00:00")),
    ).toBe("two");
    expect(
      pickStableForCurrentPeriod(choices, 7, new Date("2024-02-29T20:00:00")),
    ).toBe("three");
  });
});
