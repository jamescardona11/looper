...
import type { ActionCardAccent } from "./actionCardButtonAccents";
...
export function actionCardPresentation(
  ...
  const cardLayout = fullWidth || hasDescription;
  const density = cardLayout
    ? joinActionCardClasses(
        "px-3 py-2.5",
  ...
  return {
    cardLayout,
    buttonClassName: joinActionCardClasses(
    ...
    contentClassName: joinActionCardClasses(
      cardLayout
        ? "flex items-center gap-2.5"
        : "flex w-full items-center gap-1.5",
      overrides.contentClassName,
    ),
    iconClassName: joinActionCardClasses(
      cardLayout
