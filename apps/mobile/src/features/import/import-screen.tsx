  ...
  const apply = async () => {
    ...
    try {
      ...
      if (selected.has("transcripts")) {
        await Promise.all(bundle.transcripts.map(async (transcript) => {
      ...
      <ScrollView contentContainerStyle={styles.content}>
        {phase === "done" ? (
          <View style={styles.done}>
            ...
            <Text style={styles.body}>El contenido ya está disponible en Library, Studio y Ask.</Text>
          ...
          <View style={styles.preview}>
            ...
            <ImportChoice count={bundle.dictionary.length} id="dictionary" label="Diccionario" selected={selected} setSelected={setSelected} />
            <ImportChoice count={bundle.replacements.length} id="replacements" label="Reemplazos" selected={selected} setSelected={setSelected} />
            <ImportChoice count={bundle.styles.length} id="styles" label="Estilos" selected={selected} setSelected={setSelected} />
            <ImportChoice count={bundle.transcripts.length} id="transcripts" label="Historial de dictados" selected={selected} setSelected={setSelected} />
            <Text style={styles.privacy}>Solo se importa texto. Looper no copia audio ni accede a la base privada de otra app.</Text>
            <Pressable disabled={phase === "importing"} onPress={() => void apply()} style={styles.primary}>
              {phase === "importing" ? <ActivityIndicator color={colors.onAccent} /> : <Text style={styles.primaryText}>Importar selección</Text>}
            </Pressable>
          ...
          <View style={styles.hero}>
            ...
            <Pressable disabled={phase === "reading"} onPress={() => void pick()} style={styles.primary}>
              {phase === "reading" ? <ActivityIndicator color={colors.onAccent} /> : <Text style={styles.primaryText}>Elegir archivo</Text>}
            </Pressable>
          </View>
        )}
...
function ImportChoice({ id, label, count, selected, setSelected }: { id: ImportSection; label: string; count: number; selected: Set<ImportSection>; setSelected: (next: Set<ImportSection>) => void }) {
  ...
  return (
    <Pressable accessibilityRole="checkbox" accessibilityState={{ checked, disabled: count === 0 }} disabled={count === 0} onPress={toggle} style={[styles.choice, count === 0 && styles.disabled]}>
      <View style={[styles.check, checked && styles.checked]}><Text style={styles.checkText}>{checked ? "✓" : ""}</Text></View>
...
function mergeImportedStudioSettings(current: unknown, bundle: MobileImportBundle): Record<string, unknown> {
  const base = current && typeof current === "object" && !Array.isArray(current) ? current as Record<string, unknown> : {};
  const previous = Array.isArray(base.styles) ? base.styles : [];
  return { ...base, styles: [...previous, ...bundle.styles], ...(bundle.language ? { language: bundle.language } : {}) };
}
...
const styles = StyleSheet.create({
  ...
  header: { alignItems: "center", borderBottomColor: colors.border, borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: "row", minHeight: 54, paddingHorizontal: 12 },
  ...
  heroTitle: { color: colors.text, fontSize: 31, fontWeight: "700", letterSpacing: -0.9, lineHeight: 36 },
  ...
  infoCard: { backgroundColor: colors.backgroundSecondary, borderColor: colors.border, borderRadius: 15, borderWidth: 1, gap: 7, padding: 15 },
  infoTitle: { color: colors.text, fontSize: 14, fontWeight: "700" },
  choice: { alignItems: "center", backgroundColor: colors.backgroundSecondary, borderColor: colors.border, borderRadius: 13, borderWidth: 1, flexDirection: "row", gap: 11, minHeight: 54, paddingHorizontal: 13 },
  check: { alignItems: "center", borderColor: colors.borderStrong, borderRadius: 6, borderWidth: 1, height: 22, justifyContent: "center", width: 22 },
  ...
  primary: { alignItems: "center", backgroundColor: colors.accent, borderRadius: 14, justifyContent: "center", minHeight: 50, marginTop: 8 },
  primaryText: { color: colors.onAccent, fontSize: 14, fontWeight: "800" },
  secondary: { alignItems: "center", borderColor: colors.borderStrong, borderRadius: 14, borderWidth: 1, justifyContent: "center", minHeight: 48 },
