...
impl AEC {
    pub fn new() -> Result<Self, crate::Error> {
        ...
        let session_1 = looper_onnx::load_model_from_bytes(model::BYTES_1)?;
        let session_2 = looper_onnx::load_model_from_bytes(model::BYTES_2)?;
    ...
    fn run_model_1(&mut self, ctx: &mut ProcessingContext) -> Result<(), crate::Error> {
        let mut outputs = self.session_1.run(looper_onnx::ort::inputs![
        ...
        ctx.out_mask
            .copy_from_slice(out_mask_view.view().as_slice().ok_or_else(|| {
                crate::Error::ShapeError(looper_onnx::ndarray::ShapeError::from_kind(
                    looper_onnx::ndarray::ErrorKind::IncompatibleLayout,
                ))
            })?);
    ...
    fn run_model_2(&mut self, ctx: &mut ProcessingContext) -> Result<(), crate::Error> {
        let mut outputs = self.session_2.run(looper_onnx::ort::inputs![
