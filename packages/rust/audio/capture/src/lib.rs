...
pub use looper_audio_core::{CaptureConfig, CaptureFrame, CaptureStream, EchoCancelled, Error};
pub use looper_audio_interface::AsyncSource;

pub const TAP_DEVICE_NAME: &str = "looper-audio-tap";
...
impl AudioInput {
    ...
    /// Start capturing from the configured source.
    ///
    /// Opening a device can fail — it may have been unplugged, or the user may
    /// not have granted access — so callers hear about it instead of receiving
    ...
    pub fn stream(&mut self) -> Result<AudioStream, Error> {
        let stream = match &self.source {
            ...
            AudioSource::Recorded => AudioStream::Recorded {
                data: self.data.as_ref().cloned().unwrap_or_default(),
                position: 0,
            },
        };
        Ok(stream)
