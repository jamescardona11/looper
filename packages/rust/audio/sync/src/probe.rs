...
impl SyncProbe {
    ...
    /// How far the microphone lags the speaker, once the probe trusts its own
    /// measurement.
    ///
    /// `None` means do not realign — either the probe is still searching, or
    /// it has lost confidence. Callers get the decision rather than the state
    /// machine behind it: which states count as trustworthy is the probe's
    /// business, not theirs.
    pub fn alignment(&self) -> Option<isize> {
        match self.state {
            SyncProbeState::Locked | SyncProbeState::Holdover => self.stable_lag_samples,
            SyncProbeState::Searching | SyncProbeState::Acquiring | SyncProbeState::Lost => None,
        }
    }
...
mod alignment_tests {
    ...
    fn searching_probe() -> SyncProbe {
        SyncProbe::new(SyncProbeConfig::new(48_000))
    }

    #[test]
    fn a_probe_that_has_not_locked_offers_no_alignment() {
        assert_eq!(searching_probe().alignment(), None);
    }

    #[test]
    fn only_the_trusted_states_offer_an_alignment() {
        let mut probe = searching_probe();
        probe.stable_lag_samples = Some(192);

        for state in [
            SyncProbeState::Searching,
            SyncProbeState::Acquiring,
            SyncProbeState::Lost,
        ] {
            probe.state = state;
            assert_eq!(
                probe.alignment(),
                None,
                "{state:?} is not a state we realign on"
            );
        }

        for state in [SyncProbeState::Locked, SyncProbeState::Holdover] {
            probe.state = state;
            assert_eq!(probe.alignment(), Some(192), "{state:?} should realign");
        }
    }

    #[test]
    fn a_locked_probe_without_a_measurement_still_offers_nothing() {
        let mut probe = searching_probe();
        probe.state = SyncProbeState::Locked;
        probe.stable_lag_samples = None;

        assert_eq!(probe.alignment(), None);
    }
}
