...
pub trait AudioFormatExt: AsyncSource {
    ...
    {
        // Shares the band-limited resampling path with live capture, rather
        // than the linear interpolation this used to do on its own.
        let resampled = looper_resampler::ResampleExt::resampled(self, sample_rate, chunk_size);

        let chunks = match resampled {
            Ok(stream) => stream.left_stream(),
            Err(error) => {
                tracing::error!(error = %error, "could not build the resampler");
                futures_util::stream::empty().right_stream()
            }
        };

        Box::pin(chunks.chunks(chunk_size).map(|chunk| {
            let mut buf = BytesMut::with_capacity(std::mem::size_of::<i16>() * chunk.len());
            for sample in chunk {
...
pub use looper_audio_mime::content_type_to_extension;
...
// uncompilable; coverage returns with the rewrite of this module, using
// synthesised signals instead of shipped audio.
